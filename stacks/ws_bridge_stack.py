"""WS Bridge Stack — ECS Fargate service for multi-bot WebSocket bridge.

Deploys a long-running Fargate task that manages multiple DingTalk and Feishu
bot instances via WebSocket connections. Receives messages, resolves users via
DynamoDB, invokes per-user AgentCore sessions, and sends responses back.

Replaces the single-bot DingTalkStack with a unified multi-bot bridge.
...
class WsBridgeStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        cpu = int(self.node.try_get_context("ws_bridge_cpu") or 256)
        memory_mb = int(self.node.try_get_context("ws_bridge_memory_mb") or 512)

        # --- Log Group ---
        log_group = logs.LogGroup(
            self,
            "WsBridgeLogGroup",
            log_group_name="/openclaw/ws-bridge",
        ...
        # --- Security Group (egress-only: HTTPS for WebSocket + AWS APIs) ---
        sg = ec2.SecurityGroup(
            self,
            "WsBridgeSG",
            vpc=vpc,
            description="WS Bridge - egress-only for DingTalk/Feishu WebSocket and AWS APIs",
        ...
        # Secrets Manager (bot configs + channel credentials)
        task_role.add_to_policy(
            iam.PolicyStatement(
        ...
        # S3 PutObject for image/file uploads, GetObject+HeadObject for outbound
        task_role.add_to_policy(
            iam.PolicyStatement(
        ...
        cdk_nag.NagSuppressions.add_resource_suppressions(
            ...
            [
                ...
                cdk_nag.NagPackSuppression(
                    ...
                    "(ARNs, table names, region). Bot credentials are fetched at runtime "
                    "from Secrets Manager, not passed as env vars.",
                ),
            ],
